...
export const signupApi = createAsyncThunk(
  "auth/signup",
  async (
    payload: { email: string; password: string; inviter?: string },
    thunkAPI,
  ) => {
    try {
      const data = await api.post<{
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(loginApi.fulfilled, (state, action) => {
        ...
        state.status = "success";
        Cookies.set("id", action.payload.id.toString(), { expires: 7 });
        Cookies.set("token", action.payload.token.toString(), { expires: 7 });
...
export const { logout, setStatus, setUserId } = authSlice.actions;
